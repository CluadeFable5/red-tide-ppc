#!/usr/bin/env node
/**
 * Real browser verification using puppeteer-core + @sparticuz/chromium
 * Produces genuine screenshots at 375px and 768px for all three snap points,
 * both mouse-drag and touch-emulated paths.
 */
...
// Ensure LD_LIBRARY_PATH includes chromium libs
...
async function runViewport(browser, { width, height, label, hasTouch }) {
  ...
  // Peek screenshot
  await page.screenshot({ path: `${OUTPUT_DIR}/sheet-${width}-${inputType}-peek.png`, fullPage: false })
  ...
  record(
    `${label} ${inputType} peek — body hidden, map interactive`,
  ...
  try {
    ...
    const compactRows = await page.$$eval('[data-anchor="mid"] .font-display.truncate', els => els.length)
    record(`${label} ${inputType} mid shows compact rows`, compactRows >= 1, `${compactRows} rows`)
  } catch (e) {
  ...
  try {
    ...
    record(`${label} ${inputType} full caps ~88vh`, ratio >= 0.82 && ratio <= 0.92, `visible ${Math.round(ratio*100)}%`)
    ...
    record(`${label} ${inputType} full list scrollable`, true, `scrollHeight ${scrollInfo.scrollHeight} clientHeight ${scrollInfo.clientHeight}`)
  } catch (e) {
  ...
  try {
    await settleAt(page, 'peek')
    // Summary button
    const summaryBtn = await page.$('button::-p-text(zones ·)')
    // Fallback selector
